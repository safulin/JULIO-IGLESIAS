      ...
      <div class="layout">
        ...
        <main class="main-container">
          ...
          <section class="map-area">
            <div class="real-map">
              <l-map ref="mapRef" :zoom="15" :center="[41.388, 2.170]" :useGlobalLeaflet="false">
                <l-tile-layer url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png" />
                <l-polyline :lat-lngs="routeCoords" color="#4A90E2" :weight="8" />
                ...
                <l-marker v-for="(label, index) in labels" :key="'label-' + index" :lat-lng="label.position">
                  ...
                  <l-tooltip :options="{ permanent: true, direction: 'top', offset: [0, -10] }" class="google-tooltip-fixed">
                    <div class="google-label">
                      <ion-icon :icon="walkOutline" class="mode-icon"></ion-icon>
...
import { walkOutline, chevronBackOutline } from "ionicons/icons";
...
const modoSeleccionado = ref('pie');

watch(modoSeleccionado, (newVal) => {
  if (newVal === 'bus') router.push('/mapa-ruta-bus');
  if (newVal === 'coche') router.push('/mapa-ruta-coche');
});

const routeCoords = ref([[41.385, 2.173], [41.387, 2.171], [41.390, 2.168], [41.392, 2.166]]);
const labels = ref([
  { position: [41.387, 2.171], time: "23 min", dist: "1,5 km" },
  { position: [41.390, 2.168], time: "22 min", dist: "1,5 km" },
  { position: [41.392, 2.166], time: "27 min", dist: "1,8 km" },
]);
</script>
...
.mobile-only { display: none; } /* Oculto en escritorio */
